...
export function computeManualPreview(
  ...
  if (album.releaseDate !== editedState.releaseDate) {
    const changeType: ChangeType =
      !album.releaseDate && editedState.releaseDate
        ? 'ADDED'
        : album.releaseDate && !editedState.releaseDate
          ? 'REMOVED'
          : 'MODIFIED';
    ...
    const dateDiff: DateDiff = {
      field: 'releaseDate',
      changeType,
      current: album.releaseDate
        ? parseDateComponents(album.releaseDate)
        : null,
      source: editedState.releaseDate
        ? parseDateComponents(editedState.releaseDate)
        : null,
      componentChanges: {
        year: changeType,
        month: changeType,
  ...
  if (album.releaseType !== editedState.releaseType) {
    const changeType: ChangeType =
      !album.releaseType && editedState.releaseType
        ? 'ADDED'
        : album.releaseType && !editedState.releaseType
          ? 'REMOVED'
          : 'MODIFIED';
  ...
  if (album.musicbrainzId !== editedState.musicbrainzId) {
    const changeType: ChangeType =
      !album.musicbrainzId && editedState.musicbrainzId
        ? 'ADDED'
        : album.musicbrainzId && !editedState.musicbrainzId
          ? 'REMOVED'
          : 'MODIFIED';
  ...
  if (album.spotifyId !== editedState.spotifyId) {
    const changeType: ChangeType =
      !album.spotifyId && editedState.spotifyId
        ? 'ADDED'
        : album.spotifyId && !editedState.spotifyId
          ? 'REMOVED'
          : 'MODIFIED';
  ...
  if (album.discogsId !== editedState.discogsId) {
    const changeType: ChangeType =
      !album.discogsId && editedState.discogsId
        ? 'ADDED'
        : album.discogsId && !editedState.discogsId
          ? 'REMOVED'
          : 'MODIFIED';
  ...
  return {
    ...
    summary: {
      ...
      modifiedFields: fieldDiffs.filter(d => d.changeType === 'MODIFIED')
        .length,
      conflictFields: 0,
