...
interface GuessDistributionChartProps {
  /** Array of 4 integers: wins by attempt count (index 0 = 1-guess, etc.) */
  distribution: number[];
  /** If player won today, which attempt number (1-4) to highlight */
  todayAttempts?: number | null;
}

/**
 * Horizontal bar chart showing guess distribution (Wordle-style).
 * Displays wins by number of attempts (1-4).
 */
        ...
        {data.map(item => (
          <div key={item.attempt} className='flex items-center gap-2'>
            ...
            <div className='flex-1'>
              <div
                className={
                  'flex h-6 items-center justify-end rounded px-2 text-sm font-medium text-white transition-all ' +
                  (item.isToday
                    ? 'bg-green-600'
                    : item.count > 0
                      ? 'bg-zinc-600'
