...
interface SplitMosaicProps {
  ...
  onSplitPanel: (panelId: string, direction: SplitDirection, newPanel: PanelContent) => void;
...
const SplitMosaic: React.FC<SplitMosaicProps> = ({
  ...
  const [showInitialPanelSelector, setShowInitialPanelSelector] = useState(false);
  const resizeTimeoutRef = useRef<NodeJS.Timeout | null>(null);

  const handleResize = useCallback((sizes: number[], nodeId: string) => {
  ...
  const renderNode = useCallback((node: SplitNode): React.ReactNode => {
    if (node.type === 'panel') {
      return (
        <Panel
          key={node.id}
