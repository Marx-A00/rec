...
async function fetchArtistReleases(artistId: string, page: number = 1): Promise<ReleasesResponse> {
  const response = await fetch(`/api/artists/${artistId}/releases?page=${page}&per_page=25&sort=year&sort_order=desc`);
  if (!response.ok) {
    ...
    <div className="bg-zinc-800 rounded-lg p-4 hover:bg-zinc-700 transition-colors">
      <div className="flex gap-3">
        <div className="relative w-16 h-16 flex-shrink-0">
          <Image
            src={release.thumb || release.basic_information?.thumb || 'https://via.placeholder.com/64x64?text=No+Image'}
            alt={release.title}
            fill
