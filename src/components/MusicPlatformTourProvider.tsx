...
function TourStateManager() {
  const { startNextStep, isNextStepVisible, currentTour, currentStep } =
    useNextStep();
  const pathname = usePathname();
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      (window as any).debugTour = {
        ...
        testOnboarding: async () => {
          ...
          try {
            ...
            if (!statusData.isNewUser) {
              console.log(
                '🔄 User is not new - this would normally auto-trigger for new users only'
              );
            }
        ...
        resetOnboardingStatus: async () => {
          ...
          try {
            ...
            if (response.ok) {
              ...
              console.log(
                '🔄 Refresh the page to trigger auto-onboarding for testing'
              );
            } else {
              console.error(
        ...
        inspectModals: () => {
          console.log(
            '🔍 Debug: Inspecting current page modals and elements...'
          );
          ...
          closeSelectors.forEach(selector => {
            const elements = document.querySelectorAll(selector);
            console.log(
              `❌ Close selector "${selector}": ${elements.length} elements found`
            );
          ...
          console.log(
            '🎯 Current tour state:',
            tourData ? JSON.parse(tourData) : null
          );
        },
        testModalCloseAndNavigate: async () => {
          console.log(
            '🚀 Debug: Testing enhanced modal close and profile navigation...'
          );
          ...
          console.log('🔍 Browse element check:', {
            ...
            currentStepTitle:
              currentStep !== undefined
                ? musicPlatformTours.find(t => t.tour === 'welcome-onboarding')
        ...
        if (searchInput) {
          ...
          setTimeout(() => {
            const searchResults = document.querySelectorAll(
              '[data-search-result], .search-result, a[href*="/artists/"]'
            );
  ...
  useEffect(() => {
    if (pathname === '/browse') {
      ...
      if (isNextStepVisible && currentTour === 'welcome-onboarding') {
        ...
        const tourData = musicPlatformTours.find(
          tour => tour.tour === 'welcome-onboarding'
        );
        if (tourData && tourData.steps[currentStep || 0]) {
          ...
          console.log('🎯 Current step on browse page:', {
            ...
            isDiscoveryStep: currentStepData.title?.includes(
        ...
        const checkElement = () => {
          ...
          const element = document.querySelector('#browse-page-header');
          console.log(
            `🔍 Attempt ${attempts}: browse-page-header exists?`,
            !!element
          );
          ...
          if (element) {
            ...
            setTimeout(() => {
              const nextStepOverlay = document.querySelector(
                '[data-name="nextstep-overlay"]'
              );
              const nextStepCards =
                document.querySelectorAll('[data-nextstep]');
            ...
            console.log(
              '❌ Gave up looking for browse-page-header after 5 seconds'
            );
            ...
            console.log(
              '🔍 NextStep overlay visible (without element)?',
              !!nextStepOverlay
            );
          }
        };
    ...
    if (pathname.startsWith('/artists/')) {
      ...
      if (isNextStepVisible && currentTour === 'welcome-onboarding') {
        ...
        const checkElement = () => {
          ...
          console.log(
            `🔍 Attempt ${attempts}: artist-page-header exists?`,
            !!element
          );
          ...
          if (element) {
            ...
            console.log(
              '❌ Gave up looking for artist-page-header after 5 seconds'
            );
          }
        };
  ...
  useEffect(() => {
    ...
    if (
      isNextStepVisible &&
      currentTour === 'welcome-onboarding' &&
      currentStep !== undefined
    ) {
      ...
      if (tourData && tourData.steps[currentStep]) {
        console.log('🎯 Current step details:', {
          ...
          isSearchStep:
            tourData.steps[currentStep].title === 'Search for Music',
        });

        if (tourData.steps[currentStep].title === 'Search for Music') {
          console.log(
            '🔍 ✅ Confirmed on search tour step - auto-filling search with "daft punk"'
          );
          ...
          setTimeout(() => {
            ...
            if (searchInput) {
              console.log(
                '✅ Found search input, auto-filling with "daft punk"'
              );
              ...
              Object.getOwnPropertyDescriptor(
                ...
                'value'
              )?.set?.call(searchInput, 'daft punk');
              searchInput.dispatchEvent(new Event('input', { bubbles: true }));
              ...
              setTimeout(() => {
                ...
                const searchResults = document.querySelectorAll(
                  'a[href*="/artists/"], [data-search-result], .search-result-item'
                );
                console.log(
                ...
                for (let i = 0; i < searchResults.length; i++) {
                  ...
                  if (
                    ...
                    console.log(
                      '🎯 Found potential Daft Punk result, clicking:',
                      result
                    );
                    result.click();
                    return;
                ...
                console.log(
                  '❌ No Daft Punk search results found to auto-click'
                );
          ...
          const addCustomNextHandler = () => {
            ...
            if (nextButton && !nextButton.hasAttribute('data-custom-handler')) {
              console.log(
                '🎯 Adding custom Next button handler for search step'
              );
              ...
              const handleCustomNext = async (event: Event) => {
                console.log(
                  '🚀 Custom Next clicked - searching for Daft Punk artist'
                );
                ...
                try {
                  ...
                  if (searchData.results && searchData.results.length > 0) {
                    ...
                    console.log(
                      '🎯 Navigating to artist page:',
                      `/artists/${daftPunkArtist.id}`
                    );
                    window.location.href = `/artists/${daftPunkArtist.id}`;
                  } else {
                    console.log(
                      '❌ No Daft Punk artist found, allowing normal Next behavior'
                    );
  ...
  useEffect(() => {
    ...
    if (
      ...
      const checkAndStartOnboarding = async () => {
        try {
          console.log(
            '🔍 Checking onboarding status for user:',
            session.user.id
          );
 ...
 */
export function MusicPlatformTourProvider({
  children,
}: MusicPlatformTourProviderProps) {
