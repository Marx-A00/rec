...
import { ArrowLeft, Building2, Calendar, MapPin, ExternalLink } from "lucide-react";
...
export default function LabelDetailsPage() {
  ...
  const [labelDetails, setLabelDetails] = useState<any>(null);
  ...
  const fetchLabelDetails = async (id: string) => {
    ...
    try {
      ...
      setError(`Failed to fetch label details: ${err.message}`);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const fetchDetailedLabelData = async (id: string) => {
    try {
      ...
      console.log('Error fetching detailed label data:', err);
    }
  };
      ...
      <div className="min-h-screen bg-black text-white">
        <div className="container mx-auto px-4 py-8">
          ...
          <div className="text-center">
            <h1 className="text-2xl font-bold text-red-500 mb-4">Label Not Found</h1>
            <p className="text-zinc-400">{error || "The requested label could not be found."}</p>
          </div>
        </div>
      </div>
    );
      ...
      <div className="container mx-auto px-4 py-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {labelDetails?.contactinfo && (
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-zinc-300">Contact: {labelDetails.contactinfo}</span>
                </div>
              )}

              {labelDetails?.urls && labelDetails.urls.length > 0 && (
                <div className="flex items-center space-x-2">
                  <ExternalLink className="h-5 w-5 text-zinc-400" />
                  <a 
                    href={labelDetails.urls[0]} 
                    target="_blank" 
            ...
            {labelDetails?.sublabels && labelDetails.sublabels.length > 0 && (
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {labelDetails.sublabels.map((sublabel: any, index: number) => (
                    <span
                      key={index}
        ...
        <Tabs defaultValue="releases" className="w-full">
          <TabsList className="grid w-full grid-cols-4 bg-zinc-900">
            ...
            <TabsTrigger value="sublabels" className="data-[state=active]:bg-zinc-700">
              Sub-labels
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="releases" className="mt-6">
            <div className="bg-zinc-900 rounded-lg p-6">
              ...
              <p className="text-zinc-400">
                Label releases and catalog will appear here. This feature is coming soon!
              </p>
            </div>
          </TabsContent>
          ...
          <TabsContent value="artists" className="mt-6">
            <div className="bg-zinc-900 rounded-lg p-6">
              ...
              <p className="text-zinc-400">
                Artists signed to this label will appear here. This feature is coming soon!
              </p>
            </div>
          </TabsContent>
          ...
          <TabsContent value="about" className="mt-6">
            <div className="bg-zinc-900 rounded-lg p-6">
              <h3 className="text-xl font-semibold mb-4">About {label.title}</h3>
              {labelDetails?.profile ? (
                <div className="text-zinc-300 leading-relaxed">
                  {labelDetails.profile.split('\n').map((paragraph: string, index: number) => (
              ...
              {labelDetails?.contactinfo && (
                <div className="mt-6 p-4 bg-zinc-800 rounded-lg">
                  ...
                  <p className="text-zinc-300 text-sm">{labelDetails.contactinfo}</p>
                </div>
              )}
            </div>
          </TabsContent>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {labelDetails.sublabels.map((sublabel: any, index: number) => (
                    <div key={index} className="bg-zinc-800 p-4 rounded-lg">
                      <h4 className="font-semibold text-white">{sublabel.name}</h4>
