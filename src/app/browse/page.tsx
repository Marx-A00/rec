...
async function getRecommendations(): Promise<Recommendation[]> {
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_URL || 'http://localhost:3000'}/api/recommendations`, {
      cache: 'no-store'
    });
...
export default function BrowsePage() {
  ...
  if (usersError) {
    return (
      <div className="min-h-screen bg-black text-white flex items-center justify-center">
        <div className="text-red-400">Error loading users: {usersError.message}</div>
      </div>
    );
  }
                  ...
                  <Link href={`/profile/${user.id}`}>
                    <div className="bg-zinc-900 rounded-lg p-4 hover:bg-zinc-800 transition-colors cursor-pointer border border-zinc-800">
                      <div className="flex flex-col items-center text-center">
                        ...
                        <h3 className="font-medium text-white mb-1">{user.name || 'Anonymous User'}</h3>
                ...
                >
                  ...
                  {recommendation.user && (
                    <div className="flex items-center mb-4">
                      <Avatar className="h-8 w-8 mr-3">
                        <AvatarImage src={recommendation.user.image || undefined} alt={recommendation.user.name || 'User'} />
                        <AvatarFallback className="bg-zinc-700 text-zinc-200 text-xs">
                          {recommendation.user.name?.charAt(0) || 'U'}
                        </AvatarFallback>
                      </Avatar>
                      <div>
                        <p className="text-sm font-medium text-white">{recommendation.user.name || 'Anonymous User'}</p>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center space-x-3">
                      <img 
                        src={recommendation.basisAlbumImageUrl || "https://via.placeholder.com/48x48?text=No+Image"} 
                    ...
                    <div className="flex items-center space-x-3">
                      <img 
                        src={recommendation.recommendedAlbumImageUrl || "https://via.placeholder.com/48x48?text=No+Image"} 
