...
const mockAlbums = [
  {
    src: { title: 'OK Computer', artist: 'Radiohead', hue: 'bg-sky-900' },
    rec: { title: 'Kid A', artist: 'Radiohead', hue: 'bg-red-900' },
    score: 10,
  },
  {
    src: { title: 'In Rainbows', artist: 'Radiohead', hue: 'bg-orange-900' },
    rec: {
      title: 'Loveless',
  ...
  {
    src: { title: 'The Bends', artist: 'Radiohead', hue: 'bg-indigo-900' },
    rec: { title: 'Dummy', artist: 'Portishead', hue: 'bg-amber-900' },
    score: 8,
  },
  {
    src: { title: 'Amnesiac', artist: 'Radiohead', hue: 'bg-slate-800' },
    rec: { title: 'Vespertine', artist: 'Bjork', hue: 'bg-violet-900' },
    score: 7,
  },
  {
    src: {
    ...
    rec: { title: 'Homogenic', artist: 'Bjork', hue: 'bg-teal-900' },
    score: 6,
  },
  {
    src: { title: 'Pablo Honey', artist: 'Radiohead', hue: 'bg-yellow-900' },
    rec: {
      title: 'Siamese Dream',
...
const options: ColorOption[] = [
  {
    name: 'Current',
    description:
      'Pastel from-*-50 to-*-50 with border-*-100 — nearly invisible on dark bg',
...
function AlbumPlaceholder({ hue }: { hue: string }) {
  return (
    <div
      className={`w-full aspect-square rounded-lg ${hue} flex items-center justify-center`}
    >
...
function MockCard({
  ...
  const bgClass =
    solid && colors.bgSolid
      ? colors.bgSolid
      : `bg-linear-to-r ${colors.bgGradient || ''}`;
      ...
      <div className='relative'>
        ...
        <div className='absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-20'>
          <div className='bg-black border-3 border-black rounded-full shadow-lg'>
            <div
              className={`flex items-center justify-center w-10 h-10 ${bgClass} rounded-full border-2 ${colors.borderColor} shadow-md`}
            >
              <div className='flex flex-col items-center'>
                <Heart
                  className={`h-3 w-3 ${colors.heartColor} drop-shadow-xs mb-0.5`}
                />
                <span
                  className={`text-xs font-bold ${colors.textColor} tabular-nums leading-none`}
                >
                  {album.score}
                </span>
              </div>
...
function MockModalBadge({
  ...
  return (
    <div className='bg-black border-2 border-black rounded-full shadow-xl'>
      <div
        className={`flex items-center justify-center w-16 h-16 ${bgClass} rounded-full border-2 ${colors.borderColor} shadow-lg`}
      >
        <div className='flex flex-col items-center'>
          <Heart
            className={`h-5 w-5 ${colors.heartColor} drop-shadow-xs mb-0.5`}
          />
          <span
            className={`text-sm font-bold ${colors.textColor} tabular-nums leading-none`}
          >
            {score}
          </span>
        </div>
  ...
  return (
    <div className='min-h-screen bg-zinc-950 p-8'>
      ...
      <p className='text-zinc-400 mb-10'>
        Each option shown in card context (small badge) and modal context (large
        badge)
      </p>
