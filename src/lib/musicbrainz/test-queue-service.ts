...
async function testQueuedMusicBrainzService() {
  ...
  try {
    ...
    console.log('First result:', artists[0]?.name, '-', artists[0]?.disambiguation);
    ...
    const releases = await musicbrainzQueueService.searchReleaseGroups('OK Computer', 3);
    ...
    console.log('Making 3 concurrent searches - should be processed sequentially...');
    ...
    console.log('Results:', concurrentResults.map((result, index) => 
      `${index + 1}: ${result.length} artists`
    ).join(', '));
