// Minor type issues with BullMQ, needs cleanup
...
async function debugJobCompletion() {
  ...
  const { processMusicBrainzJob } = await import('./musicbrainz-processor');
  ...
  try {
    ...
    console.log(
      `Waiting: ${waiting.length}, Active: ${active.length}, Completed: ${completed.length}, Failed: ${failed.length}`
    );
    ...
    if (completed.length > 0) {
      ...
      for (const job of lastCompleted) {
        ...
        console.log(
          `  - Finished: ${job.finishedOn ? new Date(job.finishedOn) : 'Not finished'}`
        );
        console.log(`  - Return value exists: ${!!job.returnvalue}`);
        console.log(`  - Return value type: ${typeof job.returnvalue}`);
        console.log(
          `  - Return value sample:`,
          JSON.stringify(job.returnvalue, null, 2)?.substring(0, 200) + '...'
        );
