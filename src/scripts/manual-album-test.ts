...
async function testAlbumEnrichment() {
  ...
  console.log(
    `🎵 Testing: "${testAlbum.title}" by ${testAlbum.artists[0].artist.name}`
  );
  ...
  try {
    ...
    if (searchResults.length > 0) {
      ...
      if (bestMatch) {
        console.log(
          `🏆 Best Match: "${bestMatch.result.title}" (Score: ${(bestMatch.score * 100).toFixed(1)}%)`
        );
        console.log(
          `🎯 Exceeds 80% threshold: ${bestMatch.score > 0.8 ? '✅ YES' : '❌ NO'}`
        );
