...
7. Also update CHECK_ALBUM_ENRICHMENT spawned jobs (if any) similarly.
   ...
   <verify>
8. handleEnrichAlbum signature accepts Job<EnrichAlbumJobData>
9. ENRICH_ARTIST jobs spawned include parentJobId: rootJobId
10. All log entries include jobId, parentJobId, isRootJob
    </verify>
...
5. Update all enrichmentLogger.logEnrichment calls to include:
   ...
   <verify>
6. handleEnrichArtist signature accepts Job<EnrichArtistJobData>
7. DISCOGS_SEARCH_ARTIST and CACHE_ARTIST_IMAGE jobs include parentJobId
8. All log entries include jobId, parentJobId, isRootJob
   </verify>
...
4. handleEnrichTrack:
   ...
   - Add logging with jobId, parentJobId, isRootJob
     ...
     <verify>
5. All 6 handler signatures in enrichment-processor.ts accept Job<T> parameter
6. All spawned jobs include parentJobId: rootJobId
7. pnpm type-check passes (may still have errors from other processors)
   </verify>
