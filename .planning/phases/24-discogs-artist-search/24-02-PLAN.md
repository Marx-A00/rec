...
<task type="auto">
  <name>Task 1: Add source parameter to artistCorrectionSearch schema</name>
  <files>
    src/graphql/schema.graphql
  </files>
  <action>
1. Find the artistCorrectionSearch query definition in schema.graphql:
   ```graphql
...
4. Add source field to ArtistCorrectionSearchResult type if not present:
   ```graphql
...
Schema validates: `pnpm codegen` succeeds
  ...
  <done>
artistCorrectionSearch query accepts source parameter with MUSICBRAINZ default.
  </done>
</task>
