...
<task type="auto">
  <name>Task 3: Add searchArtists method to QueuedDiscogsService</name>
  <files>
    src/lib/discogs/queued-service.ts
  </files>
  <action>
...
3. Add searchArtists method to QueuedDiscogsService class (after getMaster method):
    ...
    *
    * Uses existing DISCOGS_SEARCH_ARTIST job type which is already
    * implemented in the enrichment pipeline. The job returns search
    * results that we map to correction format.
    *
   ...
   async searchArtists(
     ...
     try {
       ...
       // The handler is optimized for enrichment (single best match),
       // but also returns search results that we can use
       const job = await this.queue.addJob(
         JOB_TYPES.DISCOGS_SEARCH_ARTIST,
         {
           ...
           // Signal this is a correction search (optional, for future use)
           isCorrectionSearch: true,
         },
         {
       ...
       const result = await this.waitForJobViaEvents(job.id!) as {
         searchResults?: Array<{ id: number; title: string; score?: number }>;
         action?: string;
         resultsCount?: number;
       };

       // Extract search results from job result
       // The handler returns searchResults array with raw Discogs data
       const searchResults = result.searchResults || [];

       // Map to ArtistSearchResult format
       const mappedResults = searchResults
         .slice(0, limit)
         .map(r => mapDiscogsSearchResultToArtistSearchResult(r, r.score));
...
4. The method uses existing DISCOGS_SEARCH_ARTIST job type, maps results via the new mapper, and returns in the expected format for the GraphQL resolver.

IMPORTANT: The existing job handler may need modification to return ALL search results (not just best match). This will be handled as a follow-up if needed - the job already logs searchResults.length so the data is available.
  </action>
  <verify>
TypeScript compiles: `pnpm type-check`
File contains searchArtists method: `grep -q "searchArtists" src/lib/discogs/queued-service.ts`
  ...
  <done>
QueuedDiscogsService.searchArtists() method implemented, calls existing DISCOGS_SEARCH_ARTIST job, maps results to ArtistSearchResult format.
  </done>
</task>
