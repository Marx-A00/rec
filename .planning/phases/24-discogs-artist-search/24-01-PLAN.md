...
This follows the same pattern as DISCOGS_SEARCH_ALBUM which returns `results: []` array.
  ...
  <verify>
TypeScript compiles: `pnpm type-check`
Grep confirms searchResults in all return paths:
  - `grep -c "searchResults:" src/lib/queue/processors/discogs-processor.ts` should show 3 (one per return statement)
  - `grep -A2 "action: 'no_results'" src/lib/queue/processors/discogs-processor.ts | grep searchResults`
  - `grep -A2 "action: 'no_confident_match'" src/lib/queue/processors/discogs-processor.ts | grep searchResults`
  - `grep -A2 "action: 'found_and_queued'" src/lib/queue/processors/discogs-processor.ts | grep searchResults`
  </verify>
  <done>
DISCOGS_SEARCH_ARTIST handler returns searchResults array in all 3 return paths for correction UI consumption.
  </done>
</task>
