...
3. Add apply mutation with callbacks:
   - On success: show "Applied!" state, show toast with summary, invalidate queries, auto-close after 1.5s
   - On error: show toast with error message
   - Build summary from changes object (fields count, tracks count)

4. Handle apply action:
   - Convert UI selections to GraphQL format using toGraphQLSelections
   - Call mutation with albumId, releaseGroupMbid, selections, expectedUpdatedAt
...
4. Clicking "Confirm & Apply" triggers mutation with correct input
5. Success shows "Applied!" state, toast with summary, auto-closes after 1.5s
6. Error shows inline in ApplyView, admin can retry
7. Modal state cleared on successful apply
8. Album queries invalidated on success
</verification>
