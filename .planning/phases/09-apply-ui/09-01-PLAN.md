...
1. **UIFieldSelections interface** - Matches apply service FieldSelections but uses simpler structures for UI state:
   ...
   **Note on artist selection:** The backend FieldSelections includes an `artists: Map<string, boolean>` field for per-artist selection. This UI intentionally does NOT expose individual artist selection because:
   - Artist credits are typically applied as a unit from MusicBrainz (the source of truth)
   - Per-artist selection adds UI complexity without clear user benefit
   - The apply service will apply all artist changes when any fields are selected

   If per-artist selection is needed later, add `artists: { applyAll: boolean; excludedMbids: Set<string> }` following the tracks pattern.
...
3. **toGraphQLSelections(ui: UIFieldSelections, preview: CorrectionPreview)** - Converts UI state to GraphQL input format:
   ...
   - For artists: Pass all artists from preview.artistDiff.source as selected=true (apply as unit)
