...
must_haves:
  truths:
    ...
    - 'Preview shows diff between original and edited values (in combined Preview+Apply step)'
...
**3. Add resolver to mutations.ts with full artist update implementation:**
...
manualCorrectionApply: async (_, { input }, { prisma, user }) => {
  ...
  if (album.updatedAt.toISOString() !== input.expectedUpdatedAt) {
    return {
      success: false,
      error: { code: 'STALE_DATA', message: 'Album was modified' },
    };
  }

  // Parse release date
  const releaseDate = input.releaseDate
    ? parsePartialDate(input.releaseDate)
    : null;
  ...
  const updated = await prisma.$transaction(async tx => {
    ...
    // Pattern from apply-service.ts: delete all, then upsert+create new associations

    // Delete all existing artist associations for this album
    await tx.albumArtist.deleteMany({ where: { albumId: input.albumId } });
...
**5. Update CorrectionModal.tsx for 3-step manual mode flow:**

Manual mode uses 3 steps per CONTEXT.md decision: "Two-step flow: Manual Edit step → Preview+Apply combined step"

- Step 0: Current Data (with "Edit Manually" button)
- Step 1: ManualEditView (edit fields)
- Step 2: Combined Preview+Apply (shows diff AND apply button)
...
b) Update StepIndicator for manual mode (3 steps vs 4 for search):
...
<StepIndicator
  ...
  steps={
    isManualMode
      ? ['Current Data', 'Edit', 'Apply'] // 3 steps for manual
      : ['Current Data', 'Search', 'Preview', 'Apply'] // 4 steps for search
  }
/>
```

c) Add "Manual Edit" button to Current Data step (step 0):

- Add button below CurrentDataView: "Edit Manually" with Pencil icon
- On click: setIsManualMode(true), setCurrentStep(1)
...
e) Handle manual preview - compute diff and go to combined Preview+Apply step:
...
f) Render combined Preview+Apply for manual mode at step 2:

```tsx
{
  currentStep === 2 && isManualMode && manualPreviewData && (
    <div className='space-y-6'>
      {/* Preview section - reuse PreviewView components for diff display */}
      <div className='border border-zinc-800 rounded-lg p-4'>
        <h3 className='text-lg font-medium mb-4'>Preview Changes</h3>
        {/* Render fieldDiffs from manualPreviewData */}
        {/* Reuse existing diff components: TextDiffDisplay, DateDiffDisplay, ArtistDiffDisplay */}
        <FieldDiffsDisplay diffs={manualPreviewData.fieldDiffs} />
      </div>
...
const handleManualApply = async () => {
  ...
  try {
    setIsApplying(true);
    await manualCorrectionApplyMutate({
      input: {
        albumId: album.id,
    ...
    const changedCount = manualPreviewData?.fieldDiffs.length ?? 0;
    toast.success(
...
i) Clear manual state on close/apply success
</action>
<verify>Run pnpm codegen, then pnpm type-check. Open modal, click "Edit Manually", edit title, click "Preview Changes" - shows diff in combined step. Click Apply - uses new mutation with correct artist update, applies with manual_correction source.</verify>
<done>Complete manual edit flow works end-to-end with 3 steps (Current Data, Edit, Preview+Apply combined), dedicated GraphQL mutation with proper artist handling via AlbumArtist join table</done>
</task>
...
4. Manual testing flow:
   ...
   c. Verify StepIndicator shows 3 steps: "Current Data", "Edit", "Apply"
   d. Edit title, add/remove artist, change release date
   e. Try to preview with invalid field - blocked with errors
   f. Fix validation errors, click "Preview Changes"
   g. Verify combined step shows diff (current vs edited) AND Apply button
   h. Click "Apply" - changes applied to database with manual_correction source
   i. Verify artists updated correctly in AlbumArtist join table
...
- StepIndicator shows 3 steps for manual mode (not 4)
- Form validation blocks preview until all fields valid
- Combined Preview+Apply step shows both diff and apply button (satisfies MANUAL-05)
- New manualCorrectionApply mutation exists and works
- Artist updates use AlbumArtist join table pattern (delete all, then upsert+create)
