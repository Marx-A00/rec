...
must_haves:
  truths:
    - "DailyChallenge, GameSession, GameGuess, PlayerGameStats tables exist in database"
    - "GameSession links to User and DailyChallenge via foreign keys"
    - "GameGuess links to GameSession via foreign key"
    - "PlayerGameStats links to User with 1:1 relationship"
    - "Prisma client includes all game-related types"
  artifacts:
    - path: "prisma/schema.prisma"
      provides: "Game models (DailyChallenge, GameSession, GameGuess, PlayerGameStats) and GameStatus enum"
      contains: "model DailyChallenge"
    - path: "prisma/migrations/*_add_daily_game_models"
      provides: "Migration SQL for game tables"
  key_links:
    - from: "GameSession"
      to: "User"
      via: "userId relation"
    ...
    - from: "GameSession"
      ...
      pattern: "challenge\\s+DailyChallenge\\s+@relation"
    - from: "GameGuess"
      to: "GameSession"
      via: "sessionId relation"
      pattern: "session\\s+GameSession\\s+@relation"
...
<objective>
Add Prisma models for the daily album art game: DailyChallenge, GameSession, GameGuess, and PlayerGameStats.
...
Output: Four new database tables with proper relations, indexes, and a GameStatus enum.
</objective>
...
<task type="auto">
  <name>Task 1: Add game models to Prisma schema</name>
...
3. **Update the User model to add game relations:**
...
4. **Update the Album model to add game relations:**
...
- SetNull for optional Album reference in GameGuess (guessedAlbum)
  </action>
  <verify>
Run `pnpm prisma validate` to check schema syntax is correct. Should output "The Prisma schema is valid."
  </verify>
  <done>
Schema file contains all four game models (DailyChallenge, GameSession, GameGuess, PlayerGameStats), GameStatus enum, and User/Album relations. Prisma validates successfully.
  </done>
</task>

<task type="auto">
  <name>Task 2: Run migration and verify types</name>
  <files>prisma/migrations/*_add_daily_game_models/migration.sql</files>
  <action>
1. Run the Prisma migration to create the tables:
```bash
pnpm prisma migrate dev --name add_daily_game_models
```

This will:
- Generate migration SQL in `prisma/migrations/YYYYMMDDHHMMSS_add_daily_game_models/`
...
This confirms the Prisma client types are correctly generated and the project compiles.
  ...
  <verify>
1. `pnpm prisma migrate dev --name add_daily_game_models` completes without errors
2. Migration file exists at prisma/migrations/*_add_daily_game_models/migration.sql
...
2. **Relations correct:** Schema shows GameSession -> User, GameSession -> DailyChallenge, GameGuess -> GameSession relations
3. **Prisma generates:** `pnpm prisma generate` succeeds (auto-ran with migrate dev)
4. **Types available:** `pnpm type-check` confirms DailyChallenge, GameSession, GameGuess, PlayerGameStats types compile

Quick database verification (optional, using Supabase MCP):
```sql
SELECT table_name FROM information_schema.tables 
WHERE table_schema = 'public' 
AND table_name IN ('daily_challenges', 'game_sessions', 'game_guesses', 'player_game_stats');
```
Should return 4 rows.
</verification>

<success_criteria>
1. `prisma/schema.prisma` contains DailyChallenge, GameSession, GameGuess, PlayerGameStats models
2. `prisma/schema.prisma` contains GameStatus enum at end of file
3. User model has `gameSessions` and `gameStats` relations
4. Album model has `dailyChallenges` and `gameGuesses` relations
5. Migration file exists at `prisma/migrations/*_add_daily_game_models/`
