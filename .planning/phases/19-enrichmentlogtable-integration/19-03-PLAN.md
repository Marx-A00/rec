...
must_haves:
  ...
  key_links:
    - from: "src/components/admin/EnrichmentLogTable.tsx"
      ...
      via: "parentOnly: true in main query variables"
...
@src/generated/graphql.ts (updated in plan 01 with parentOnly)
</context>
...
- Children lazy-loaded via separate query per expanded row
- SkeletonTimeline shown while loading
- EnrichmentTimelineModal for full view
- Row count shows "X jobs (Y total)" format
...
3. **Extract ExpandableLogRow as a separate component** (defined inside the same file, below the main component). This component handles per-row child fetching:
   ...
   function ExpandableLogRow({ log, isExpanded, onToggle }: ExpandableLogRowProps) {
     // Lazy fetch children only when expanded
     const { data: childrenData, isLoading: loadingChildren, error: childrenError, refetch: refetchChildren } =
       useGetEnrichmentLogsQuery(
         {
           parentJobId: log.jobId,  // NOTE: This needs the parentJobId filter - see note below
           limit: 100,
         },
         {
   ...
   **IMPORTANT NOTE on child fetching:** The current GraphQL query does NOT have a `parentJobId` filter variable for fetching children by parent. We need a workaround. Two options:

   **Option A (PREFERRED):** Use the `GetEnrichmentLogsWithChildren` query for the main table query instead. Pass `includeChildren: true` to get children bundled with each parent log. But this defeats lazy loading...

   **Option B (RECOMMENDED):** Since the resolver's flat fetch doesn't filter by `parentJobId` as a query parameter, we need to fetch children differently. The simplest approach: use the EXISTING `includeChildren` approach but ONLY when a row is expanded. Create a separate query call using `useGetEnrichmentLogsWithChildrenQuery` with the parent's `entityType` and `entityId`, then find the matching parent in the results to get its children.

   **Option C (SIMPLEST - USE THIS):** Actually, look at the resolver carefully. The flat fetch can be filtered by any field in the `where` clause. But `parentJobId` is not currently a filter parameter in the GraphQL schema. Since Plan 01 only added `parentOnly: Boolean`, NOT a `parentJobId: String` filter... We should use a DIFFERENT approach for fetching children:

   For each expanded row, use `useGetEnrichmentLogsWithChildrenQuery` with `entityType`, `entityId`, and extract the children from the matching parent log. The query returns parent logs with nested children, so we:
   1. Call `useGetEnrichmentLogsWithChildrenQuery({ entityType: log.entityType, entityId: log.entityId })`
   2. Find the parent log in results that matches `log.id`
   3. Extract `children` from that parent
   4. Use `enabled: isExpanded` to lazy-load

   This is slightly less efficient but works with the existing GraphQL layer without additional schema changes. The `includeChildren` resolver already does the batch child fetch efficiently.

   ```typescript
   const { data: treeData, isLoading: loadingChildren, error: childrenError, refetch: refetchChildren } =
     useGetEnrichmentLogsWithChildrenQuery(
       {
       ...
       },
       {
         ...
         refetchInterval: (query) => {
           ...
           const matchingParent = logs.find(l => l.id === log.id);
           const children = matchingParent?.children || [];
           if (children.length === 0) return false;
           const lastChild = children[children.length - 1];
           const age = Date.now() - new Date(lastChild.createdAt).getTime();
           return age < 30000 ? 3000 : false;
   ...
   const matchingParent = treeData?.enrichmentLogs.find(l => l.id === log.id);
   const children = (matchingParent?.children || []) as EnrichmentLog[];
   ```
...
6. **Update row count display** to show parent count with indication of nested children:
   ```typescript
   <span className="text-xs text-zinc-500 font-normal">
     ({logs.length} {logs.length === 1 ? 'job' : 'jobs'})
   </span>
   ```
   (Simple change from "logs" to "jobs" terminology. A separate total count would require an additional query which is unnecessary for now.)
...
8. **Add import for useGetEnrichmentLogsWithChildrenQuery:**
   ```typescript
   import {
     useGetEnrichmentLogsQuery,
     useGetEnrichmentLogsWithChildrenQuery,
   ...
   ```
   (Also import `type EnrichmentLog` if not already imported.)

**Type casting note:** The `log` objects from `useGetEnrichmentLogsQuery` and `useGetEnrichmentLogsWithChildrenQuery` may have slightly different shapes (one has `children`, one doesn't). Cast to `EnrichmentLog` type where needed to satisfy the EnrichmentTimeline props. The `EnrichmentLog` type from generated graphql should work. If there are type mismatches, create a minimal adapter or use type assertions.
...
- Keep the loading state, error state, and empty state for the main table unchanged
  ...
  <done>
    ...
    Children lazy-loaded via useGetEnrichmentLogsWithChildrenQuery when expanded.
...
4. Verify the following by reading the final file:
   ...
   - `useGetEnrichmentLogsWithChildrenQuery` is used for child fetching
...
6. Remove unused imports (ArrowRight, Edit if no longer used after FieldChangesPanel removal).
  </action>
  <verify>
...
5. All rows expandable (chevron on every row)
6. Expanded rows show compact timeline or skeleton loading
7. Error state with retry exists for failed child fetches
8. EnrichmentTimelineModal renders in expanded area
9. Solo logs show as single-item timeline
10. No unused imports or dead code warnings
</verification>
