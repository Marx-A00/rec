...
export class DiffEngine {
  ...
  classifyChange(current: unknown, source: unknown): ChangeType {
    ...
    const hasSourceValue = source !== null && source !== undefined && source !== '';
    ...
    if (typeof current === 'string' && typeof source === 'string') {
      return TextNormalizer.areEqual(current, source) ? 'UNCHANGED' : 'CONFLICT';
    }
   ...
   */
  compareText(field: string, current: string | null, source: string | null): TextDiff {
    ...
    if (changeType === 'CONFLICT' && current && source) {
      ...
      result.parts = changes.map((part): TextDiffPart => ({
        value: part.value,
  ...
  compareDate(current: Date | null, source: string | null): DateDiff {
    ...
    const changeType = this.classifyDateChange(currentComponents, sourceComponents);
    ...
    return {
      ...
      componentChanges: {
        year: this.classifyChange(currentComponents?.year, sourceComponents?.year),
        month: this.classifyChange(currentComponents?.month, sourceComponents?.month),
   ...
   */
  private classifyDateChange(current: DateComponents | null, source: DateComponents | null): ChangeType {
  ...
  compareArray(field: string, current: string[], source: string[]): ArrayDiff {
    ...
    const currentNormalized = new Map(current.map(item => [TextNormalizer.normalize(item), item]));
    const sourceNormalized = new Map(source.map(item => [TextNormalizer.normalize(item), item]));
    ...
    if (added.length > 0 && removed.length === 0 && current.length === 0) {
      changeType = 'ADDED';
    } else if (removed.length > 0 && added.length === 0 && source.length === 0) {
      changeType = 'REMOVED';
  ...
  compareArtistCredits(
    ...
    source: Array<{ name: string; joinphrase?: string; artist: { id: string; name: string } }>
  ): ArtistCreditDiff {
    // Format display strings
    const currentDisplay = current.map(c => c.name).join(', ');
    const sourceDisplay = source.map((s, i) => {
      const name = s.name;
    ...
    if (changeType === 'CONFLICT' && currentDisplay && sourceDisplay) {
      ...
      result.nameDiff = changes.map((part): TextDiffPart => ({
        value: part.value,
        added: part.added || undefined,
        removed: part.removed || undefined,
      }));
   ...
   */
  compareTracks(currentTracks: Track[], sourceMediums: MBMedium[]): {
    ...
    for (const track of sourceTracks) {
      if (!sourceByDisc.has(track.discNumber)) sourceByDisc.set(track.discNumber, []);
    ...
    for (const discNum of [...allDiscs].sort((a, b) => a - b)) {
      ...
      const maxLen = Math.max(currentDiscTracks.length, sourceDiscTracks.length);
      ...
      for (let i = 0; i < maxLen; i++) {
        ...
        if (currTrack && srcTrack) {
          ...
          const titlesEqual = TextNormalizer.areEqual(currTrack.title, srcTrack.title);
          ...
          if (!titlesEqual) {
            diff.titleDiff = diffChars(currTrack.title, srcTrack.title).map(part => ({
              value: part.value,
              added: part.added || undefined,
              removed: part.removed || undefined,
            }));
          ...
          if (!durationsEqual && currTrack.durationMs !== null && srcTrack.durationMs !== null) {
            diff.durationDelta = Math.abs(currTrack.durationMs - srcTrack.durationMs);
          }
...
// Normalizers
export { TextNormalizer, normalizeForComparison, parseDateComponents, formatDateComponents } from './normalizers';
