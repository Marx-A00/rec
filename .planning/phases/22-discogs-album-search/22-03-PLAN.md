     ...
     useSearchCorrectionCandidatesQuery(
       {
         input: {
           ...
           limit: 10,
           offset: searchOffset,
           source: correctionSource === 'discogs' 
...
4. If there's a separate Discogs query variable (discogsQuery), remove it — we now use single unified query with source parameter.
