...
must_haves:
  truths:
    ...
    - "Resolver routes to Discogs service when source is DISCOGS"
  ...
  key_links:
    - from: "src/lib/graphql/resolvers/queries.ts"
      to: "src/lib/discogs/album-search-service.ts"
      via: "getDiscogsAlbumSearchService import"
      pattern: "getDiscogsAlbumSearchService"
...
Output: GraphQL schema with source parameter, resolver routing to Discogs service.
</objective>
...
1. Add import at top of file:
   ```typescript
   import { getDiscogsAlbumSearchService } from '@/lib/discogs/album-search-service';
   ```
...
3. Add conditional routing based on source. Before the existing MusicBrainz search logic:
   ...
   if (source === 'DISCOGS') {
     const discogsService = getDiscogsAlbumSearchService();
     const discogsResponse = await discogsService.search({
...
Codegen succeeds: `pnpm codegen`
  ...
  <done>
Resolver routes to Discogs service when source is DISCOGS, returns same GraphQL shape as MusicBrainz.
  </done>
</task>
