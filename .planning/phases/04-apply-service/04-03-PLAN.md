...
2. **Singleton Pattern** (for Next.js HMR safety)
   ```typescript
   const globalForApply = globalThis as unknown as { applyCorrectionService?: ApplyCorrectionService };
   export const applyCorrectionService = globalForApply.applyCorrectionService ?? new ApplyCorrectionService();
   if (process.env.NODE_ENV !== 'production') globalForApply.applyCorrectionService = applyCorrectionService;
   ```
