...
must_haves:
  truths:
    ...
    - "PRIORITY_TIERS imported from @/lib/queue in preview-service.ts"
  artifacts:
...
4. Add import for PRIORITY_TIERS:
import { PRIORITY_TIERS } from '@/lib/queue';

5. Update generatePreview method signature to accept source parameter with default:
...
6. Inside generatePreview, add conditional fetch based on source. Replace the existing mbReleaseData fetch with:
...
7. Update generateFieldDiffs call to pass source:
