...
must_haves:
  truths:
    - "Each result shows title, artist, year, track count, release type"
...
3. Content structure:
   ...
   - Row 2: Artist + Year (text-sm text-zinc-400 truncate)
     Format: `{primaryArtistName} • {firstReleaseDate?.split('-')[0]}`
   - Row 3: Metadata (text-xs text-zinc-500)
     - primaryType (if present)
     - trackCount (if present, format: "• {n} tracks")
...
Component renders result data correctly
  ...
  <done>
SearchResultCard displays thumbnail, title with score, artist/year, metadata row with [MB] badge, full row clickable
  </done>
</task>
...
- SearchResultCard displays all required fields (title, artist, year, tracks, type, score, [MB] badge)
