...
const { data, isLoading, error } = useSearchCorrectionCandidatesQuery(
  ...
  {
    enabled: isSearchTriggered && !!(searchQuery.albumTitle || searchQuery.artistName),
  }
);
```
...
6. Render logic:
   ...
   - Otherwise: render SearchInputs + (results ? SearchResults : initial prompt)

7. Initial state (before first search):
...
SearchView exports correctly
  ...
  <done>
SearchView wires SearchInputs, SearchResults, GraphQL query, and modal state persistence
  </done>
</task>
...
5. Replace step 1 placeholder with SearchView:
```tsx
{/* Step 1: Search */}
...
7. Handle album data availability:
   - SearchView needs album prop
   - Only render when album is loaded
  </action>
  <verify>
pnpm type-check passes
Modal renders SearchView at step 1
  </verify>
  <done>
CorrectionModal renders SearchView at step 1, result selection navigates to preview step
  </done>
</task>
...
4. Results display with all required metadata
