...
7. **Keep in ArtistCorrectionModal (do NOT move to store):**
   ...
   **Final ArtistApplyView prop interface:**
   ...
   const handleApply = () => {
     ...
     if (!artistId || !state?.previewData || !state?.selectedArtistMbid || !state?.applySelections) return;
...
3. **Get artistId and artistMbid from store context.** The parent modal (ArtistCorrectionModal) knows the artistId. For PreviewView to get it with zero props, it needs a way to know which store to read. 
   ...
   **Pragmatic decision (matching album pattern):** Keep `artistId` as a single prop for store lookup. The requirement "zero props" means zero *state* props — artistId is an identity/key prop. This matches Phase 13 where albumId was kept as identity prop (see STATE.md: "albumId as identity prop: PreviewView and ApplyView keep albumId as prop (needed to locate store instance)").
...
5. **Replace onPreviewLoaded callback with store action:**
   ...
   useEffect(() => {
     if (data?.artistCorrectionPreview) {
       store.getState().setPreviewLoaded(data.artistCorrectionPreview as ArtistCorrectionPreview);
     }
...
const handleApply = () => {
  ...
  const expectedUpdatedAt = artistData?.updatedAt ? artistData.updatedAt : new Date();
