...
must_haves:
  truths:
    - 'Search results include a confidence score admins can use to assess match quality'
    - 'Score breakdown reveals which components (title, artist, year) matched well or poorly'
    - 'Low-confidence results are visually flagged so admins know to review carefully'
    - 'Different scoring approaches can be tested without code changes (runtime switch)'
    - 'Results are consistently sortable by match quality regardless of strategy used'
  artifacts:
...
<task type="auto">
  ...
  <action>
**Pre-check:** Verify fuzzysort is available by running:
```bash
grep -q '"fuzzysort"' package.json && echo "fuzzysort OK" || echo "ERROR: fuzzysort not in dependencies"
```
The dependency exists (used by src/lib/utils/fuzzy-match.ts). If missing, run `pnpm add fuzzysort`.
...
1. Dependency check: `grep -q '"fuzzysort"' package.json` succeeds
2. `pnpm type-check` passes
3. `pnpm lint src/lib/correction/scoring/` passes
4. Each file imports its required utilities (string-similarity, fuzzysort)
   </verify>
