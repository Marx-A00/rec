...
store.setState({ mode: 'search', step: 0, manualEditState: undefined, manualPreviewData: null });
```
...
const handleApply = () => {
  ...
  const graphqlSelections = toGraphQLSelections(state.applySelections, state.previewData);
