    ...
    try {
      ...
      for (const [index, result] of searchResults.entries()) {
        try {
          ...
          console.log(
            `  ✅ Result ${index + 1}: ${validated.name} (${validated.country || 'Unknown country'})`
          );
        } catch (error) {
      ...
      for (const validated of testResult.validatedResults) {
        try {
          ...
          console.log(
            `  🔄 Mapped: ${mapped.name} -> MBID: ${mapped.musicbrainzId}`
          );
        } catch (error) {
    ...
    try {
      ...
      for (const [index, result] of searchResults.entries()) {
        try {
          ...
          const artistNames =
            validated.artistCredit?.map(ac => ac.artist.name).join(', ') ||
            'Unknown artist';
          console.log(
            `  ✅ Result ${index + 1}: ${validated.title} by ${artistNames}`
          );

          // Test artist credit extraction
          const artistCredits = extractArtistCreditsFromReleaseGroup(validated);
          console.log(
            `    🎨 Artist credits: ${artistCredits.map(ac => `${ac.name} (pos: ${ac.position})`).join(', ')}`
          );
        } catch (error) {
          const errorMsg = `Validation failed for result ${index + 1}: ${error instanceof Error ? error.message : 'Unknown error'}`;
          testResult.errors.push(errorMsg);
      ...
      for (const validated of testResult.validatedResults) {
        try {
          ...
          testResult.mappedResults.push(mapped);
          console.log(
            `  🔄 Mapped: ${mapped.title} -> MBID: ${mapped.musicbrainzId}`
          );
          console.log(
            `    📅 Release date: ${mapped.releaseDate ? mapped.releaseDate.toISOString().split('T')[0] : 'Unknown'}`
          );
        } catch (error) {
          const errorMsg = `Mapping failed: ${error instanceof Error ? error.message : 'Unknown error'}`;
          testResult.errors.push(errorMsg);
...
async function generateTestReport(
  artistResults: TestResult[],
  albumResults: TestResult[]
) {
  ...
  const summary = {
    ...
    totalSearchResults: [...artistResults, ...albumResults].reduce(
      (sum, r) => sum + r.searchResults.length,
      0
    ),
    totalValidatedResults: [...artistResults, ...albumResults].reduce(
      (sum, r) => sum + r.validatedResults.length,
      0
    ),
    totalMappedResults: [...artistResults, ...albumResults].reduce(
      (sum, r) => sum + r.mappedResults.length,
      0
    ),
    totalErrors: [...artistResults, ...albumResults].reduce(
      (sum, r) => sum + r.errors.length,
      0
    ),
    averageProcessingTime:
      [...artistResults, ...albumResults].reduce(
        (sum, r) => sum + r.processingTime,
        0
      ) /
      (artistResults.length + albumResults.length),
    successRate: {
      validation: 0,
      mapping: 0,
    },
  ...
  summary.successRate.validation =
    totalSearchResults > 0
      ? (summary.totalValidatedResults / totalSearchResults) * 100
      : 0;
  summary.successRate.mapping =
    summary.totalValidatedResults > 0
      ? (summary.totalMappedResults / summary.totalValidatedResults) * 100
      : 0;
...
async function main() {
  ...
  try {
    ...
    console.log('\n🎉 Test Suite Complete!');
    console.log(
      `✅ Success Rate: ${summary.successRate.mapping.toFixed(1)}% mapping success`
    );
    console.log(
      `⚡ Average Time: ${summary.averageProcessingTime.toFixed(0)}ms per query`
    );

    if (summary.totalErrors > 0) {
      console.log(
        `⚠️  ${summary.totalErrors} errors encountered - check test-report.md for details`
      );
      process.exit(1);
    }
  } catch (error) {
