...
test.describe('Recommendation Drawer - Dual Input Search', () => {
  ...
  async function openRecommendationDrawer(
    ...
    // Click the "Create Recommendation" button in the navigation
    // This triggers the drawer via the RecommendationDrawerContext
    const recommendButton = page.locator('button, a').filter({
      hasText: /recommend/i,
    });

    // If we can find and click the button, do so
