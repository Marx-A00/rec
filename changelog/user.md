...
## [v1.2.0] - Uncover, Mobile & Infrastructure - June 2026

This is the biggest update since the v1.0 rewrite. A brand new game mode, a fully rebuilt mobile experience, and major infrastructure upgrades under the hood.

### New Features

#### Uncover - Album Cover Guessing Game

- **Daily Challenges**: A new album cover puzzle drops every day. Guess the album from a blurred/pixelated cover image.
- **Smart Guessing**: Text-based input with autocomplete search. Artist and decade partial-match hints help you narrow it down.

#### Mobile Experience

- **Full Mobile App**: Dedicated mobile routes for home, search, albums, artists, profiles, and settings.

NOTE: recommendation creation still not available on mobile.

#### Desktop Sidebar Redesign

- **Collapsible Sidebar**: Hamburger menu toggle with smooth expand/collapse animations.
- **Back Button in Sidebar**: Navigation back button moved from inline pages to the sidebar.
- **Polished Animations**: Smooth label transitions, centered icons when collapsed, hidden scrollbar.

#### Help & Onboarding

- **Help Page Overhaul**: Replaced the old guided tour with a comprehensive FAQ page using nested accordions.
- **Demo Videos**: Help page includes video walkthroughs served from Cloudflare R2.
- **Ko-fi Support Link**: Added a support link to the help menu.

#### Authentication & Profile

- **Password Reset**: Full password reset flow with email integration via Resend.
- **Profile Image Lightbox**: Click your avatar to see it full-size.
- **Avatar Upload Improvements**: Uploads are deferred until form submit instead of uploading immediately.

### Improvements

#### Data Pipelines

- **Deezer Integration**: New Deezer editorial releases sync pipeline with playlist import for game pool.
- **ListenBrainz Integration**: Fresh releases sync from ListenBrainz with genre filtering.
- **Multi-Source Latest Releases**: Latest releases page now pulls mainly from Deezer, but ListenBrainz also available.
- **Artist Biographies**: Enriched from Wikipedia and Discogs automatically.
- **Higher-Res Cover Art**: Centralized Cover Art Archive URLs with higher resolution defaults.


#### UX Polish

- **Global Error Boundary**: Music-themed fallback UI with rotating error quotes.
- **Inline Score Editing**: Edit recommendation similarity scores directly from the card.
- **Search Persistence**: Search query persists across navigation via Zustand store.
- **Unified Image Fallbacks**: Consistent vinyl placeholder across all album image loading states.

### Bug Fixes

- Fixed timezone bug causing browse and latest releases to show wrong dates
- Fixed social feed duplicate key errors and text overflow
- Fixed soft-delete filter gaps across social feed, follows, and analytics
- Fixed marquee hover behavior (only the hovered album lifts)
- Fixed mobile catch-all route preventing 404 on desktop-only links
- Fixed game-over state persisting correctly across tab navigation
- Fixed recommendation score displaying as percentage instead of /10 on mobile
- Fixed search bar collapse state not syncing on route navigation
